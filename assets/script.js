...
function renderVendors() {
  ...
  `;
  allLi.onclick = () => { state.activeVendor = "all"; clearSearchTerm(); renderVendors(); renderFiles(); };
  ...
  state.vendors.forEach((v) => {
    ...
    `;
    li.onclick = () => { state.activeVendor = v.id; clearSearchTerm(); renderVendors(); renderFiles(); };
    vendorListEl.appendChild(li);
  });
}

// Khi kỹ sư bấm chọn 1 hãng/loại cụ thể ở sidebar hoặc tab, coi như họ muốn "duyệt" thay vì
// "tìm kiếm toàn cục" — xoá từ khoá tìm kiếm đang gõ để tránh 2 chế độ lọc xung đột nhau.
...
function getFilteredFiles() {
  ...
  // Tìm kiếm toàn cục: khi kỹ sư gõ từ khoá, bỏ qua bộ lọc Hãng/Loại đang chọn ở sidebar/tabs
  // để trả về kết quả trên TẤT CẢ các hãng (đúng nghĩa "search toàn cục"), thay vì chỉ tìm
  // trong phạm vi hãng/loại đang xem.
  if (!hasSearch) {
...
// vẫn giữ nguyên lựa chọn đang chọn nếu còn hợp lệ.
function refreshUploaderFilter() {
  const current = uploaderFilterEl.value || "all";
  const users = [...new Set(state.files.map((f) => f.user).filter(Boolean))].sort();
  uploaderFilterEl.innerHTML = '<option value="all">Tất cả người upload</option>';
  users.forEach((u) => {
    const opt = document.createElement("option");
    opt.value = u;
    opt.textContent = u;
    uploaderFilterEl.appendChild(opt);
  });
  uploaderFilterEl.value = users.includes(current) ? current : "all";
}
...
toggleAdvancedFiltersBtn.addEventListener("click", () => {
  ...
  advancedFiltersEl.style.display = isHidden ? "flex" : "none";
  toggleAdvancedFiltersBtn.classList.toggle("btn-active", isHidden);
});
